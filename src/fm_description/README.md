...
come from the `unitree_ros` repo, vendored into `src/external/` via vcs.

```
unitree_ros (vcs, gitignored)
  robots/g1_description/<variant>.urdf  +  meshes/*.STL
        │  launch reads URDF, rewrites meshes/ → file:// (container path)
        ▼
...
| `g1_root` | `…/external/unitree_ros/robots/g1_description` | dir holding the URDF + `meshes/` |
...
cannot resolve alone. The launch rewrites them to absolute `file://` URIs and
widens the bridge's `asset_uri_allowlist` to the mesh directory. foxglove_bridge
reads the files inside the container and streams the bytes to Studio, so the host
never needs the mesh files.
