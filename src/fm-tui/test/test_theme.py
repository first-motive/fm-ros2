...
def test_resolver_matches_nish_tui_availability():
    if _nish_tui_installed():
        ...
        assert theme.Header.__module__ == "fm_tui.widgets"
        assert theme.LogView.__module__ == "fm_tui.widgets"


def test_widgets_share_api_across_both_sets():
    # Drop-in swap contract: the resolved widgets carry the API app.py relies on.
