...
| `data_engine`, `pipeline`, `recorder`, `capture_lifecycle`, `dataset_process`, `disposition`, `emit_gates`, `rig_topology` | `src/fm_data` (private overlay) | recording through to an emitted dataset |
| `train_serve` | `fm_policy` (private, standalone — outside this workspace) | training a policy and serving inference |
