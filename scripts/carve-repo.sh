...
ASSETS="${ASSETS:-$SRC/scripts/carve-assets/$NAME}"
ORG="${ORG:-first-motive}"

# Validate the names that reach the shell / gh CLI, so a stray value can't inject
# flags or commands into `gh repo create/edit`.
[[ "$NAME" =~ ^fm-[a-z0-9-]+$ ]] || { echo "error: bad repo name: $NAME" >&2; exit 1; }
[[ "$ORG"  =~ ^[A-Za-z0-9_.-]+$ ]] || { echo "error: bad org: $ORG" >&2; exit 1; }
PUSH="${PUSH:-0}"
OUT="${OUT:-$(mktemp -d)/$NAME}"
