...
fm_supervisor_request_exact() {
  ...
  fm_supervisor_exec bash -c "$FM_SUPERVISOR_REQUEST_EXACT_SCRIPT" _ \
    "$cmd_topic" "$yaml" "$status_topic" "$FM_SUPERVISOR_TIMEOUT" "$request_id"
}
...
# Observe one request until its queue/current/last or cloud lifecycle entry is
# terminal. It never submits another request.
...
set -u
cmd_topic=$1 yaml=$2 status_topic=$3 timeout=$4 request_id=$5
log=$(mktemp)
trap "rm -f $log" EXIT
...
def carries_id(value):
    if not isinstance(value, dict):
        return False
    if isinstance(value.get("request_id"), str) and value["request_id"] == request_id:
        return True
    for key in ("current", "last"):
        item = value.get(key)
        if isinstance(item, dict) and item.get("request_id") == request_id:
            return True
    for key in ("queue", "refused", "cloud_lifecycle"):
        items = value.get(key)
...
trap cleanup EXIT
cat >"$payload_file"
ros2 topic echo --field data "$result_topic" >"$log" 2>/dev/null &
echo_pid=$!
sleep 1
    ...
    for item in status.get("cloud_lifecycle") or []:
        if matching(item):
            ...
            if state in {"ready", "completed", "stopped", "cancelled"}:
                return 0
            if state in {"failed", "budget_blocked"}:
                return 3
            return None
    return None
while time.time() < deadline:
