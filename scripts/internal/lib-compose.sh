...
# fm_compose_transport_overlay <workspace-root>
# Write the compose layer that carries the resolved host transport into the
# container, and echo its path. Exporting variables in the host shell alone does
# not add them to a compose service or mount the Cyclone profile it names.
fm_compose_transport_overlay() {  # workspace-root
  local root="${1:?workspace root}" file
  file="$root/.fm-compose-transport.yaml"
  if [ -z "${FM_CYCLONEDDS_XML:-}" ]; then
    rm -f "$file"
    return 0
  fi
  [ -f "$FM_CYCLONEDDS_XML" ] || {
    echo "ERROR: resolved Cyclone profile is missing: $FM_CYCLONEDDS_XML" >&2
    return 1
  }
  {
    echo "# Generated by fm_compose_transport_overlay — do not edit."
    echo "services:"
    echo "  fm:"
    echo "    environment:"
    ...
    echo "    volumes:"
    printf '      - "%s:/etc/fm-comms/cyclonedds.xml:ro"\n' "$FM_CYCLONEDDS_XML"
  } > "$file"
  printf '%s\n' "$file"
}
