...
ExecStart=/bin/bash -lc 'source /opt/ros/humble/setup.bash && source $ROOT/install/setup.bash && source $ROOT/scripts/env/comms.sh && exec ros2 launch fm_tactile_bridge receiver.launch.py config:=$CONFIG_FILE'
Restart=on-failure
RestartSec=2
TimeoutStopSec=10
